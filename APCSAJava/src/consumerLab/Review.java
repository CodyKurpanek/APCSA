...
public class Review {
	...
	static {
		try {
			Scanner input = new Scanner(new File("APCSAJava/src/ConsumerLab/cleanSentiment.csv"));
		...
		try {
			Scanner input = new Scanner(new File("APCSAJava/src/ConsumerLab/negativeAdjectives.txt"));
			while (input.hasNextLine()) {
		...
		while (true) {
			...
			for (int i = 0; i < contents.length(); i ++) {
				if (contents.charAt(i) == ' ') {
					sentimentTotal += sentimentVal(contents.substring(0, i));
					System.out.println("word " + contents.substring(0, i) + " has sentiment value " + sentimentVal(contents.substring(0, i)));
			...
			if (done) {
				...
				System.out.println("word " + contents + " has sentiment value " + sentimentVal(contents));
	...
	public static int starRating(String filename) {
		...
		double a = totalSentiment(filename);
	...
	public static String fakeWord(String filename){
		String contents = (textToString(filename) + " ");
		boolean done = true;
		while (true){
			done = true;
